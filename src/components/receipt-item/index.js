...
export default class ReceiptItem extends Component {
    ...
    render() {
        ...
        return (
            <div className={"receipt-item" + (ObjectContainer.isDarkTheme() ? " dark" : "")}>
                <div>
                    <p className={"ri-currency" + (ObjectContainer.isDarkTheme() ? " dark" : "")}>{this.props.currency}</p>
                    {
                ...
                <p className={"ri-amount" + (ObjectContainer.isDarkTheme() ? " dark" : "")}>{Math.round(this.props.amount * 100) / 100}</p>
            </div>
        )
    }
